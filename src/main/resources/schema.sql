...
Create type post_status_enum as ENUM ('ACTIVE',
'NOT_ACTIVE');
...
create table if not exists "user" (
    ...
    email varchar(150),
    is_active boolean,
    activated_at timestamp DEFAULT (current_timestamp AT TIME
    ZONE 'UTC')
    );
create table comment_like (id bigserial not null,
                           comment_id int8, post_id int8, user_id int8,
                           primary key(id));
create table file (id bigserial not null, name
